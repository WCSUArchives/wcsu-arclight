...
class SolrDocument
 ...
 include Arclight::SolrDocument
  ...
  # select?fl=id&fq=component_level_isim%3A[1 TO *]&fq=level_ssm%3ACollection OR level_ssm%3Acollection&indent=true&q.op=OR&q=*%3A*&rows=100
  def collection?
    level&.parameterize == 'collection' && component_level.zero?
  end

  def unicorn
      first('repo_id_ssi') || collection&.first('repo_id_ssi')
  end
